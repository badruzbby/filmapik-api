...
**Solusi**:
1. Dockerfile telah diperbarui untuk menggunakan Rust 1.81 atau versi yang lebih baru
...
**Masalah: GLIBC versi tidak ditemukan**

Jika Anda mendapatkan error seperti:
```
/app/filmapik-api: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.33' not found (required by /app/filmapik-api)
/app/filmapik-api: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.32' not found (required by /app/filmapik-api)
/app/filmapik-api: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.34' not found (required by /app/filmapik-api)
```

**Solusi**:
1. Dockerfile telah diperbarui untuk menggunakan `debian:bookworm-slim` sebagai base image runtime yang memiliki GLIBC 2.34
2. Jalankan build ulang dengan perintah `./run-docker.sh build` atau `run-docker.bat build` untuk menerapkan perubahan
3. Jika masalah berlanjut, gunakan Docker runtime dengan glibc yang lebih baru seperti Ubuntu 22.04 atau yang lebih baru
